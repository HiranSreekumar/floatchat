...
def ingest_box(lon_min, lat_min, lon_max, lat_max, start_date, end_date):
    ...
    raw_profiles = fetch_profiles(start_date=f"{start_date}T00:00:00Z", end_date=f"{end_date}T23:59:59Z", box=box)
    ...
    with get_conn() as conn:
        ...
        for raw in raw_profiles:
            ...
            if not p["profile_id"] or not p["levels"]:
                continue
            upsert_profile(conn, p["profile_id"], p["platform_id"], p["lat"], p["lon"], p["date"], source="argo", is_synthetic=False)
...
def main():
    ...
    if args.all_regions:
        ...
        if key not in GAZETTEER:
            print(f"Unknown region '{args.region}'. Known: {list(GAZETTEER)}", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        if args.box:
            lon_min, lat_min, lon_max, lat_max = map(float, args.box.split(","))
            total += ingest_box(lon_min, lat_min, lon_max, lat_max, args.start, args.end)
        for name, (lat, lon) in regions_to_fetch:
            r = args.radius_deg
            print(f"Fetching {name} ({lat},{lon}) +/- {r} deg, {args.start}..{args.end} ...")
            n = ingest_box(lon - r, lat - r, lon + r, lat + r, args.start, args.end)
    ...
    print(f"Done. Ingested {total} real profiles. DB now has: {row_counts()}")
