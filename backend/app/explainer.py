"""
Claude call #2: real SQL results -> grounded natural-language explanation.
...
def _summarize_for_prompt(intent: QueryIntent, result: QueryResult) -> dict:
    ...
    surface_rows = [r for r in rows if r["pressure"] is not None and r["pressure"] <= 10]
    deep_rows = [r for r in rows if r["pressure"] is not None and r["pressure"] >= 500]

    def stats(vals):
        if not vals:
            return None
        return {"min": round(min(vals), 3), "max": round(max(vals), 3), "avg": round(sum(vals) / len(vals), 3), "n": len(vals)}

    return {
        "row_count": result.row_count,
        "profiles_found": len(profile_ids),
        "date_range_covered": sorted({r["profile_date"] for r in rows})[:1] + sorted({r["profile_date"] for r in rows})[-1:],
        "surface_temperature_c": stats([r["temperature"] for r in surface_rows if r["temperature"] is not None]),
        "surface_salinity_psu": stats([r["salinity"] for r in surface_rows if r["salinity"] is not None]),
        "deep_temperature_c_below_500dbar": stats([r["temperature"] for r in deep_rows if r["temperature"] is not None]),
